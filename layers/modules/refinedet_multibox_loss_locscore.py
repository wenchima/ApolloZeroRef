...
from ..box_utils_locscore import match, log_sum_exp, refine_match         ############ wenchi

import pdb      ###### wenchi
import numpy as np      ###### wenchi

class RefineDetMultiBoxLoss_Locscore(nn.Module):
    ...
    def __init__(self, num_classes, overlap_thresh, prior_for_matching,
        ...
        super(RefineDetMultiBoxLoss_Locscore, self).__init__()
    ...
    def forward(self, predictions, targets):
        ...
        locscore_t.requires_grad = False
        ...
        pos_idx_ls = pos.unsqueeze(pos.dim()).expand_as(locscore_data)
        locscore_p = locscore_data[pos_idx_ls].view(-1, 1)
        locscore_t = locscore_t.unsqueeze(2)
        locscore_t = locscore_t[pos_idx_ls].view(-1, 1)
        locscore_t = locscore_t.float()                                     # for smooth l1 loss
        loss_ls = F.smooth_l1_loss(locscore_p, locscore_t, reduction='sum')
        #locscore_t = locscore_t.squeeze(1)
        #loss_ls = F.cross_entropy(locscore_p, locscore_t, reduction='sum')
        #pdb.set_trace()
        #loss_ls = F.cross_entropy(loss_ls, locscore_t.view(1,-1).squeeze(1), reduction='sum')
        #############################################
        #print('aft', loss_c)
        #print(pos)
        #print(loss_c.shape)
