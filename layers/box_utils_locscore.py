...
def refine_match(threshold, truths, priors, variances, labels, loc_t, conf_t, locscore_t, idx, arm_loc=None):
    ...
    if arm_loc is None:
        ...
        loc = encode(matches, priors, variances)
        locscore = best_truth_overlap         # wenchi
    else:
        conf = labels[best_truth_idx] + 1     # Shape: [num_priors]
    ...
    conf_t[idx] = conf  # [num_priors] top class label for each prior
    #print("!!!", conf_t)
    #a = np.where(conf_t == 1)        ######### wenchi
    #print("***", a)
    locscore_t[idx] = locscore  # wenchi
