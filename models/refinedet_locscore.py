...
class RefineDet(nn.Module):
    ...
    def __init__(self, phase, size, base, extras, ARM, ODM, TCB, num_classes):
        ...
        self.arm_locscore = nn.ModuleList(ARM[2])      ######## wenchi
        self.odm_loc = nn.ModuleList(ODM[0])
        self.odm_conf = nn.ModuleList(ODM[1])
        self.odm_locscore = nn.ModuleList(ODM[2])      ######## wenchi
    ...
    def forward(self, x):
        ...
        arm_loc = list()
        arm_conf = list()
        arm_locscore = list()         ############ wenchi
        odm_loc = list()
        odm_conf = list()
        odm_locscore = list()         ############ wenchi
        ...
        for (x, l, c, ls) in zip(sources, self.arm_loc, self.arm_conf, self.arm_locscore):         ########## wenchi
            arm_loc.append(l(x).permute(0, 2, 3, 1).contiguous())
            arm_conf.append(c(x).permute(0, 2, 3, 1).contiguous())
            arm_locscore.append(ls(x).permute(0, 2, 3, 1).contiguous())                            ########## wenchi
        arm_loc = torch.cat([o.view(o.size(0), -1) for o in arm_loc], 1)
        arm_conf = torch.cat([o.view(o.size(0), -1) for o in arm_conf], 1)
        arm_locscore = torch.cat([o.view(o.size(0), -1) for o in arm_locscore], 1)                 ########## wenchi
        ...
        for (x, l, c, ls) in zip(tcb_source, self.odm_loc, self.odm_conf, self.odm_locscore):          ############ wenchi
            odm_loc.append(l(x).permute(0, 2, 3, 1).contiguous())
            odm_conf.append(c(x).permute(0, 2, 3, 1).contiguous())
            odm_locscore.append(ls(x).permute(0, 2, 3, 1).contiguous())                                ############ wenchi
        odm_loc = torch.cat([o.view(o.size(0), -1) for o in odm_loc], 1)
        odm_conf = torch.cat([o.view(o.size(0), -1) for o in odm_conf], 1)
        odm_locscore = torch.cat([o.view(o.size(0), -1) for o in odm_locscore], 1)                     ############ wenchi
        ...
        if self.phase == "test":
            ...
            output = self.detect(
                ...
                self.softmax(arm_conf.view(arm_conf.size(0), -1,
                             2)),                               # arm conf preds
                self.softmax(arm_locscore.view(arm_locscore.size(0), -1,
                             1)),                               # arm locscore preds                   ############# wenchi
                odm_loc.view(odm_loc.size(0), -1, 4),           # odm loc preds
                self.softmax(odm_conf.view(odm_conf.size(0), -1,
                             self.num_classes)),                # odm conf preds
                self.softmax(odm_locscore.view(odm_locscore.size(0), -1,
                             1)),                               # odm locscore preds                   ############## wenchi
            ...
            output = (
                ...
                arm_locscore.view(arm_locscore.size(0), -1, 1),                                        ############## wenchi
                odm_loc.view(odm_loc.size(0), -1, 4),
                odm_conf.view(odm_conf.size(0), -1, self.num_classes),
                odm_locscore.view(odm_locscore.size(0), -1, 1),                                        ############## wenchi
                self.priors
            )
        return output
...
def arm_multibox(vgg, extra_layers, cfg):
    ...
    for k, v in enumerate(vgg_source):
        ...
        arm_locscore_layers += [nn.Conv2d(vgg[v].out_channels,            ############# wenchi
                        cfg[k] * 1, kernel_size=3, padding=1)]            ############# wenchi
    for k, v in enumerate(extra_layers[1::2], 3):
        ...
        arm_locscore_layers += [nn.Conv2d(v.out_channels, cfg[k]          ############# wenchi
                                  * 1, kernel_size=3, padding=1)]         ############# wenchi
    #return (arm_loc_layers, arm_conf_layers)
    return (arm_loc_layers, arm_conf_layers, arm_locscore_layers)         ############# wenchi

def odm_multibox(vgg, extra_layers, cfg, num_classes):
    odm_loc_layers = []
    odm_conf_layers = []
    odm_locscore_layers = []              ############# wenchi
    vgg_source = [21, 28, -2]
    for k, v in enumerate(vgg_source):
        ...
        odm_locscore_layers += [nn.Conv2d(256, cfg[k] * 1, kernel_size=3, padding=1)]             ############### wenchi
    for k, v in enumerate(extra_layers[1::2], 3):
        odm_loc_layers += [nn.Conv2d(256, cfg[k] * 4, kernel_size=3, padding=1)]
        odm_conf_layers += [nn.Conv2d(256, cfg[k] * num_classes, kernel_size=3, padding=1)]
        odm_locscore_layers += [nn.Conv2d(256, cfg[k] * 1, kernel_size=3, padding=1)]             ############### wenchi
    #return (odm_loc_layers, odm_conf_layers)
    return (odm_loc_layers, odm_conf_layers, odm_locscore_layers)                                 ############### wenchi
